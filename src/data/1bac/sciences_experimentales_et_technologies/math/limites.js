  ...
  parts: [
    {
      ...
      definition: "La limite d'une fonction f en un point a est la valeur vers laquelle tend f(x) lorsque x se rapproche de a. On note lim(x→a) f(x) = L. On distingue les limites finies et infinies, ainsi que les limites à gauche (x→a⁻) et à droite (x→a⁺).",
      example: "La fonction f(x) = 1/x a pour limite 0 quand x tend vers l'infini: lim(x→∞) 1/x = 0. Elle n'a pas de limite en 0 car lim(x→0⁺) 1/x = +∞ et lim(x→0⁻) 1/x = -∞.",
      gameQuestions: [
    ...
    },
    {
      ...
      definition: "Si lim f(x) = a et lim g(x) = b (limites finies), alors: lim(f+g) = a+b, lim(f×g) = a×b, lim(f/g) = a/b si b≠0. Ces règles s'étendent aux cas où certaines limites sont infinies, avec des précautions pour les formes indéterminées comme 0/0, ∞/∞, 0×∞.",
      example: "Si lim f(x) = 3 et lim g(x) = 2, alors lim(f(x)×g(x)) = 3×2 = 6 et lim(f(x)/g(x)) = 3/2. Si lim f(x) = +∞ et lim g(x) = 5, alors lim(f(x)+g(x)) = +∞.",
      gameQuestions: [
    ...
    },
    {
      ...
      definition: "Certaines fonctions usuelles ont des limites connues: lim(x→+∞) x^n = +∞ pour n>0, lim(x→+∞) 1/x^n = 0 pour n>0. Pour les fonctions trigonométriques: sin et cos sont bornées entre -1 et 1, lim(x→0) sin(x)/x = 1.",
      example: "lim(x→+∞) √x = +∞, lim(x→+∞) e^x = +∞, lim(x→-∞) e^x = 0, lim(x→+∞) ln(x) = +∞, lim(x→0⁺) ln(x) = -∞",
      gameQuestions: [
